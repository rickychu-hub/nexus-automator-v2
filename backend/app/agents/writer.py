...
def agent_technical_writer(nodes, user_request, model):
    ...
    prompt = (
        ...
        f"End_Note_Content\n\n"
        f"**--- SECCIÓN 2: AUDITORÍA TÉCNICA (Manual de Configuración) ---**\n"
        f"Genera un bloque MARKDOWN ÚNICO llamado 'SPEC_SHEET_START' hasta 'SPEC_SHEET_END'.\n"
        f"Estructura OBLIGATORIA (Itera sobre cada nodo complejo):\n\n"
        f"## 🛠️ Auditoría de Configuración (Nodo a Nodo)\n\n"
        f"### [N] [Nombre del Nodo] (Tipo: [Tipo])\n"
        f"* **Estado:** 🔴 Requiere Credencial / 🟡 Revisar Parámetro / 🟢 Configurado\n"
        f"* **📝 Función:** [Una línea explicando qué hace].\n"
        f"* **⚙️ Qué configurar aquí:**\n"
        f"    * **Credencial:** [Instrucción EXACTA sobre qué credencial conectar, ej: 'OpenAI API'].\n"
        f"    * **Parámetros Críticos:**\n"
        f"        * Campo `[Nombre]`: [Instrucción validación].\n"
        f"        * *Ejemplo:* \"Campo `Prompt`: Verifica que incluya `{{{{ $json.title }}}}`\".\n"
        f"        * *Ejemplo:* \"Campo `Channel`: Cambia `YOUR_CHANNEL` por tu ID real\".\n"
        f"* **🔗 Wiring Check (Cables):** [Verifica input/output, ej: 'Este nodo espera recibir `json.text` del nodo anterior'].\n"
        f"---\n"
        f"Nota final: Recuerda marcar explícitamente cualquier placeholder (YOUR_API_KEY, etc) que el usuario DEBA cambiar.\n"
    )
